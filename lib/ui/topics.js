...
function adjustQuestionVotes(upamount, downamount) {

    var currentQuestionUpvotes   = $('.question_total').data('upvotes');
    var currentQuestionDownvotes = $('.question_total').data('downvotes');
    console.log(currentQuestionUpvotes, currentQuestionDownvotes);
    console.log(upamount, downamount);
    currentQuestionUpvotes += upamount;
    currentQuestionDownvotes += downamount;
...
function handleDisplay(topicAssets) {
    var upvotechanges   = 0;
    var downvotechanges = 0;
    var myQuestionVote = null;
    _.each(topicAssets, function(asset) {
        ...
        if (assetDoc.type === 'topic-vote')   {
            ...
            if (assetDoc._deleted) {
                console.log('deleted');
                if (assetDoc.vote_type === 'downvote') downvotechanges--;
                if (assetDoc.vote_type === 'upvote') upvotechanges--;
            } else {
                if (assetDoc.vote_type === 'upvote')  {
                    upvotechanges++;
                    if (assetDoc.vote_type_prev === 'downvote') downvotechanges--;
                }
                if (assetDoc.vote_type === 'downvote') {
                    downvotechanges++;
                    if (assetDoc.vote_type_prev === 'upvote') upvotechanges--;
                }
            }
            if (currentContext && currentContext.name === assetDoc.user) {
                if (assetDoc._deleted) myQuestionVote = null;
                else myQuestionVote = assetDoc.vote_type;
            }
        }
    ...
    showVotes(upvotechanges, downvotechanges, myQuestionVote);
}

function showVotes(upvotechanges, downvotechanges, myvote) {
    adjustQuestionVotes(upvotechanges, downvotechanges);
