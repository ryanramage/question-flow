...
exports.getUserReputations = function(userIds, callback) {
    db.getView('topicflow', 'userReputationShort', {
        keys : userIds,
        reduce : true,
        group: true
    }, callback);
...
exports.new = function() {
    ...
    // get the tags
    db.getView('topicflow', 'all_tags', function(err, result) {
...
function voteQuestion(id, user, user_reputation, vote_type, callback) {
    ...
    $.ajax({
        url :  '_db/_design/topicflow/_update/voteQuestion/' + vote_hash + '?topic=' + id + '&vote_type=' + vote_type + '&user_reputation=' + user_reputation,
        type: 'PUT',
...
function voteAnswer(question_id, answer_id, user, user_reputation, vote_type, callback) {
    ...
    $.ajax({
        url :  '_db/_design/topicflow/_update/voteAnswer/' + vote_hash + '?topic=' + question_id + '&answer=' + answer_id + '&vote_type=' + vote_type + '&user_reputation=' + user_reputation,
        type: 'PUT',
        success : function(result) {
            var err = null;
            if (result !== common.voteSuccessMessage) {
