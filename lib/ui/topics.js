                ...
                if (!promptLogin()) {
                    if ($('.topic-show .upvote').hasClass('myvote')) {
                        exports.removevoteQuestion(id, currentContext.name, function (err, data) {
                            ...
                            adjustQuestionVotes(-1, 0);
                        });
                    } else {
                        exports.upvoteQuestion(id, currentContext.name, function (err, data) {
                            if (err) return alert(err);
                            var downamount = 0;
                            if ($('.topic-show .downvote').hasClass('myvote')) downamount = -1;
                            adjustQuestionVotes(1, downamount);
                            $('.topic-show .upvote').addClass('myvote');
                ...
                if (!promptLogin()) {
                    if ($('.topic-show .downvote').hasClass('myvote')) {
                        exports.removevoteQuestion(id, currentContext.name, function (err, data) {
                            if (err) return alert(err);
                            $('.topic-show .downvote').removeClass('myvote');
                            adjustQuestionVotes(0, -1);
                        });
                    } else {
                        exports.downvoteQuestion(id, currentContext.name, function (err, data) {
                            if (err) return alert(err);
                            var upamount = 0;
                            if ($('.topic-show .upvote').hasClass('myvote')) upamount = -1;
                            adjustQuestionVotes(upamount, 1);
                            $('.topic-show .downvote').addClass('myvote');
                            $('.topic-show .upvote').removeClass('myvote');
                        });
                    }
                }
...
function adjustQuestionVotes(upamount, downamount) {
    ...
    console.log(currentQuestionDownvotes, currentQuestionUpvotes);
    ...
    console.log(currentQuestionDownvotes, currentQuestionUpvotes, votesTotal);
    $('.question_total')
...
function handleDisplay(topicAssets) {
    ...
    _.each(topicAssets, function(asset) {
        ...
        if (assetDoc.type === 'topic-vote')   {
            if (assetDoc.vote_type === 'upvote')  currentQuestionUpvotes++;
            else currentQuestionDownvotes++;
            if (currentContext && currentContext.name === assetDoc.user) {
                myQuestionVote = assetDoc.vote_type;
            }
        }
