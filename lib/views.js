...
exports.userReputation = {
    map : function(doc) {
        if (!doc.user_reputation) return;
        if (doc.type === 'topic-vote')   {
            if (doc.vote_type == 'upvote') {
                emit([doc.user_reputation, doc.timestamp, 1], 5);
            }
            if (doc.vote_type == 'downvote') {
                emit([doc.user_reputation, doc.timestamp, 0], -2);
            }
        }
        if (doc.type === 'answer-vote')  {
            if (doc.vote_type == 'upvote') {
                emit([doc.user_reputation, doc.timestamp, 1], 10);
            }
            if (doc.vote_type == 'downvote') {
                emit([doc.user_reputation, doc.timestamp, 0], -2);
                emit([doc.user, doc.timestamp, 0], -1);
            }
        }
        if (doc.type == 'topic') {
            if (doc.accepted) {
                emit([doc.user_reputation, doc.accepted_timestamp, 2], 2);
            }
        }
        if (doc.type == 'answer') {
            if (doc.accepted) {
                emit([doc.user, doc.accepted_timestamp, 2], 15);
            }
        }
...
exports.userReputationShort = {
    map : function(doc) {
        if (!doc.user_reputation) return;
        if (doc.type === 'topic-vote')   {
            if (doc.vote_type == 'upvote') {
                emit(doc.user_reputation, 5);
            }
            if (doc.vote_type == 'downvote') {
                emit(doc.user_reputation, -2);
            }
        }
        if (doc.type === 'answer-vote')  {
            if (doc.vote_type == 'upvote') {
                emit(doc.user_reputation, 10);
            }
            if (doc.vote_type == 'downvote') {
                emit(doc.user_reputation, -2);
                emit(doc.user, -1);
